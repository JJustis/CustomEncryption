...
class MandelbrotBlockchainCipher {
    ...
    public function readBlockchainStorage() {
        ...
        if ($zip->open(BLOCKCHAIN_ZIP) === TRUE) {
            // Try to extract with password - note ZipArchive::setPassword instead of setEncryptionName for extraction
    ...
    private function writeBlockchainStorage($data) {
        ...
        try {
            ...
            if ($zip->open(BLOCKCHAIN_ZIP, $mode) === TRUE) {
                ...
                // Check if encryption method constants are available (PHP 7.2+)
                if (defined('ZipArchive::EM_AES_256')) {
                    // Modern approach - use encryption method constant
                    $zip->setEncryptionName(BLOCKCHAIN_FILE, ZipArchive::EM_AES_256, ZIP_PASSWORD);
                } else {
                    // Backward compatible approach - just set password
                    $zip->setPassword(ZIP_PASSWORD);
                }
            ...
            // Create zip without built-in encryption using command line tools if available
                            ...
                            <div class="col-md-6">
                                ...
                                <div>
                                    ...
                                    <ul class="small">
                                        ...
                                        <li>Cross-platform compatible ZIP encryption</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
